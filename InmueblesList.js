  ...
  const handleEdit = async (id) => {
    try {
      ...
      if (inmueble && !inmueble.error) {
        navigate(`/editarInmueble/${id}`, { state: { inmueble } }); // Pasar los datos al componente de edición
      } else {
          ...
          {inmuebles.length > 0 ? (
            inmuebles.map((inmueble) => (
              <div className="inmueble-card" key={inmueble.idInmueble}>
                <img src={inmueble.ImagenUrl || 'default-image.jpg'} alt={inmueble.Nombre} className="inmueble-image" />
