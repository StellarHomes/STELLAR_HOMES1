...
const LoginForm = () => {
  ...
  const [userName, setUserName] = useState(""); // Guardamos el nombre del usuario
  const [userEmail, setUserEmail] = useState(""); // Guardamos el email del usuario
  const [userId, setUserId] = useState(""); // Guardamos el ID del usuario
  ...
  const validateForm = async () => {
    ...
    try {
      ...
      if (response.ok && result.message) {
        setUserName(result.user_name);  // Guarda el nombre del usuario
        setUserEmail(result.user_email); // Guarda el correo del usuario
        setUserId(result.userId); // Guarda el ID del usuario
        ...
        if (loginType === "user") {
          ...
          navigate("/indexUsu"); // Redirige a la página de usuario
          ...
          navigate("/perfil"); // Redirige a la página de inmobiliaria
        }
      } else {
        Swal.fire({
          icon: 'error',
  ...
  // Este es el manejador para el enlace "Iniciar Sesión"
