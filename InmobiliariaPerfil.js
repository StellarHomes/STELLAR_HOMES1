...
const InmobiliariaPerfil = () => {
  ...
  useEffect(() => {
    fetch('http://localhost/API/Inmobiliaria.php') // URL corregida
      .then((response) => {
        if (!response.ok) {
      ...
      .then((data) => {
        ...
        setInmobiliariaData(data); // Actualiza el estado con los datos del registro
        setLoading(false);
      })
      .catch((error) => {
  ...
  const handleSubmit = (e) => {
    ...
    fetch('http://localhost/API/EditInmobiliaria.php', {
      ...
      body: JSON.stringify(inmobiliariaData), // Asegúrate de que contiene todos los campos
    })
